cordova.define("device-testdemo.tests", function(require, exports, module) { 
...
exports.defineAutoTests = function() {
  describe('Device Information (window.device)', function () {
    ...
    it("should contain a platform specification that is a string", function() {
      ...
      expect((new String(window.device.platform)).length = 0).not.toBe(true);
    });

    it("should contain a version specification that is a string", function() {
      expect(window.device.version).toBeDefined();
      expect((new String(window.device.version)).length > 0).toBe(true);
      expect((new String(window.device.version)) == '4.2.2').toBe(true);
    });
...
exports.defineManualTests = function(contentEl, createActionButton) {
  var logMessage = function (message, color) {
    ...
    createActionButton('Dump device', function() {
      clearLog();
      logMessage(JSON.stringify(window.device, null, '\t'),'blue');
    }, "dump_device");
};
